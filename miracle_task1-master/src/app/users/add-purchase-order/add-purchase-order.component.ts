...
export class AddPurchaseOrderComponent implements OnInit {
  ...
  reactiveForm = this.formBuilder.group({
    ...
    EmpID: ['', [ Validators.maxLength(15),Validators.pattern(/^[0-9]*$/)]],
    Originzation: ['' ],
    Swift_Timings: [''],
    Status: ['', ],
    doj: ['', ],
    ...
    Personal_Phone: ['', [ Validators.pattern("^((\\+91-?)|0)?[0-9]{10}$"),Validators.maxLength(10)]],
    Cooparate_mail: ['', [ Validators.email,Validators.pattern("^[a-z0-9._%+-]+@[a-z0-9.-]+\\.[a-z]{2,4}$")]],
    ...
    State: ['', ],
    City: ['', ],
    zip: ['', [Validators.pattern(/^-?(0|[1-9]\d*)?$/),Validators.maxLength(6)]],
    Facebook: ['',],
    LinkedIn: ['', ],
    Twitter: ['',]
  });
  ...
  ngOnInit(): void {
    ...
    // console.log(today2);
    // this.reactiveForm.get('doj')?.patchValue(this.formatDate(new Date()));
  }
